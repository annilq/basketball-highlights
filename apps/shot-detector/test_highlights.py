...
def test_generate_highlights():
    ...
    model_path = "best.pt"
    video_path = "video_test_5.mp4"
    output_path = "test_highlights.mp4"
    ...
    try:
        ...
        # Generate highlights
        highlights_path = detector.generate_highlights(video_path, output_path=output_path)
        
        # Check if the highlights video was created
        if os.path.exists(highlights_path):
            print(f"✓ Success! Highlights video generated at: {highlights_path}")
            print(f"  Video size: {os.path.getsize(highlights_path) / 1024 / 1024:.2f} MB")
            return True
        else:
            print(f"✗ Error: Highlights video not found at expected path: {highlights_path}")
            return False
